...
- **Autosave**: every mutation calls `scheduleSave()` (1.1 s debounce → PATCH); `snapshot()` deduplicates unchanged saves. `render()` does *not* save — a mutation without an explicit `scheduleSave()` is lost. Don't add a separate save path.
