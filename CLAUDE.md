...
- `index.html` — admin login + project list (inline CSS/JS). The editor, loaded at `/p/<id>`, is split into `editor.html` (markup + inline SVG `<symbol>` icons), `editor.css` (design tokens in `:root`) and `editor.js` (one IIFE, no modules; `$()`/`$$()` helpers). `permab-io.js` (`window.PermabIO`) is shared by both pages: `buildExport(state)` builds the self-contained `.permab.json` (images as data URLs; same shape as the old standalone app's files, so `scripts/import-permab.js` and both pages accept old files), `importInto(pid, data)` re-uploads the images then PATCHes everything. Filenames are `Nom_AAAA-MM-JJ_HHhMM.permab.json`.
...
- **Tools are exclusive** — the `TOOLS` registry (`select`, `plant`, `zone`, `pond`, `ditch`, `path`, `item`, `ruler`, `calib`, plus the pseudo-tool `import` whose hint is the replace-project confirmation/progress; `noImage:true` lets it run on an empty project) gives each tool a `shortcut`, `enter/exit`, `onClick/onMove/onKey` (or `drag:true` + `dragStart/Move/End` for the pencil, where Space+drag pans instead) and a `hint()` (content of the contextual tag at the top of the stage, rebuilt by `updateHint()`; supports `input`/`select`/`actions`). `setTool(name)` is the only way to switch; it stamps `#stage[data-tool]`, and CSS uses that to turn off `pointer-events` on markers/shapes whenever the tool isn't `select` — so a new drawing tool is non-clickable-through by default. `Escape` → select tool (or deselect), letter keys switch tools, `Delete`/`Backspace` deletes the selection.
- **Colors**: `palette()` returns the project palette (`state.palette`, JSONB column `palette`; `null` = the default `COLORS`). `colorPicker(container, name, current, onPick(color, live))` renders swatches + pencil (OS `<input type="color">`, live preview via `live=true`) + a "+" button; changing a swatch never recolors other elements. Never index `COLORS` directly for defaults — use `palette()`.
